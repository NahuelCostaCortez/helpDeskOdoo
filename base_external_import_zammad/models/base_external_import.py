...
class Task(models.Model):
    ...
    #                               required=True, default=lambda self: self.env['byta.support.ticket'])
    ...
    def _import_data(self, flds, data, model_obj, table_obj, log):
        ...
        importmsg = dict() # creating a dictionary
        try:
            importmsg = model_obj.load(cols, [data])
            _logger.debug('Mensaje de importacion %s...' % importmsg)
        except:
    ...
    #ids=None es un argumento opcional, que si no se le da valor valdrá None
    def import_run(self, ids=None):

        # ids value depends where the function is called.
        run_ids = None
        # con isinstance se comprueba el tipo de dato que es
		# Si es un diccionario
        if isinstance(ids, dict): 
            run_ids = self.ids
        # Si es solo 1 id (de tipo entero)
        ...
        for action_ref in actions:
            ...
            if not obj.enabled:
                continue # Con continue se le indica que no se traten aquellos objetos que tengan el enabled a false
            ...
            params = [(sync)]
            # En res está el resultado de la consulta
            ...
            for row in res['rows']:
                ...
                _logger.info('El valor de cols es = %s' % cols)
                # Build data row;
                # import only columns present in the "cols" list
                data = list()
                #Recorrer con los values del diccionario
                for v in dic.values():
                    if v == dic['priority']:
                        if v == '1 low':
                            v = 'Baja'
                    ...
                    if v == dic['state']:
                        ...
                        if v == 'new':
                            ...
                            v = 'esperando_respuesta'

                    if v == dic['partner_id']:
                        search_partner = self.env['res.partner'].sudo().search([('email', '=', dic['email'])])
                        if len(search_partner) > 0:
                            dic['partner_id'] = search_partner[0].id
                        else:
                            dic['partner_id'] = self.env['ir.model.data'].get_object('helpdesk_bytacora', 'byta_ticket_partner_unassigned')

                    _logger.info('El valor de responsible_user es  = %s' % dic['responsible_user'])
                    if v == dic['responsible_user']:
                        search_partner = self.env['res.partner'].sudo().search([('id', '=', dic['responsible_user'])])
                        search_user = self.env['res.users'].sudo().search([('partner_id', '=', search_partner.id)])
                        if len(search_user) > 0:
                            dic['responsible_user'] = search_user[0].id
                        else:
                            dic['responsible_user'] = self.env['ir.model.data'].get_object('helpdesk_bytacora', 'byta_ticket_user_unassigned')

                        _logger.info('El valor que se le asigna a responsible_user es  = %s' % dic['responsible_user'])
                ...
                #_logger.info('data vale %s' % data)
                self._import_data(dic.keys(), data, model_obj, obj, log)
